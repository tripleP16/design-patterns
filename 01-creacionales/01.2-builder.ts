...
class QueryBuilder {
  ...
  execute(): string {
    // Select id, name, email from users where age > 18 and country = 'Cri' order by name ASC limit 10;
    const selectedFields = this.fields.length > 0 ? `SELECT ${this.fields.join(', ')}`: 'SELECT *';
    const whereClasue = this.conditions.length > 0 ? `WHERE ${this.conditions.join(' AND ')}`: '';
   ...
   return `${selectedFields} FROM ${this.table} ${whereClasue} ${orderBy} ${limitClause};`
  }
}
